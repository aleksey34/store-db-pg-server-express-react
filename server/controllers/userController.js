...
class UserController{
    ...
    async login(req , res, next){
        ...
        if(!user){
            return next(ApiError.badRequest('User is not existed!'))
        }

        const comparePassword =  bcrypt.compareSync(password , user.password  );
        if(!comparePassword){
            return next(ApiError.badRequest("Password is wrong , try again"));
        }
        const token = generateJWT(user.id , user.email , user.role);

        return res.json( { token });

        // return res.json({message: "test api user auth -- api/user/login"});
